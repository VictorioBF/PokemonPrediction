public class State {
    ...
    public String identity() {
        return p1.name + "_" + hp1 + "_" + p2.name + "_" + hp2;
    }
    ...
    public State step(int move1Index, int move2Index) {
        State next = new State(p1, p2, hp1, hp2);

        Move move1 = p1.moves.get(move1Index);
        Move move2 = p2.moves.get(move2Index);

        Logger.logSeparator();
        Logger.log(p1.name + " (" + hp1 + "%)");
        Logger.log(p2.name + " (" + hp2 + "%)");
        Logger.log(p1.name + " escolheu: " + move1.name);
        Logger.log(p2.name + " escolheu: " + move2.name);
    ...
    private void apply(
        ...
        if (move.category == Category.STATUS) {
            ...
            if (attackerIsP1) {
                ...
                Logger.log("HP: " + before + "% -> " + next.hp1 + "%");
            } else {
                int before = next.hp2;
                next.hp2 = Math.min(100, next.hp2 + 50);
                Logger.log("HP: " + before + "% -> " + next.hp2 + "%");
            }
        ...
        for (Type t : defender.types) {
            effectiveness *= DamageCalculator.effectiveness(move.type, t);
        }

        int damage = DamageCalculator.calculateDamage(attacker, defender, move);

        Logger.log(attacker.name + " usou " + move.name);
        Logger.log("Dano: " + damage + "%");

        if (effectiveness > 1) {
            Logger.log("Super efetivo!");
        } else if (effectiveness < 1 && effectiveness > 0) {
            Logger.log("Pouco efetivo!");
        } else if (effectiveness == 0) {
            Logger.log("Sem efeito!");
        }

        if (attackerIsP1) {
            int before = next.hp2;
            next.hp2 = Math.max(0, next.hp2 - damage);
            Logger.log(defender.name + ": " + before + "% -> " + next.hp2 + "%");
        } else {
            int before = next.hp1;
            next.hp1 = Math.max(0, next.hp1 - damage);
            Logger.log(defender.name + ": " + before + "% -> " + next.hp1 + "%");
        }

        if (next.hp1 <= 0) {
            Logger.log(p1.name + " foi derrotado!");
        }

        if (next.hp2 <= 0) {
            Logger.log(p2.name + " foi derrotado!");
        }
    }
}
